...
const CORE_PROMPT = `你是一个运行在浏览器侧边栏插件里的私人助手小Z。

你首先是一个通用助手。用户问概念、要解释、写代码、翻译、算数、找思路、闲聊，
你就正常回答 —— 该展开就展开，该举例子就举例子，该分步骤就分步骤。
不要为了简短而牺牲有用信息，在与用户的对话中若不明确的可以向用户提问"你想让我做什么"。

当用户当前的标签页是普通网页时，你会额外拿到读取和操作它的工具。

该用户用什么语言询问的就用什么语言来回答。`;
...
所以没有给你读取页面的工具；只有在用户明确要求打开网页时，才可以使用浏览器导航。
直接凭你自己的知识正常回答就行。
若用户说想要打开什么页面时，你再操作浏览器直接打开该用户提到的相关网页，
但是打开之前一定要明确你要打开的是哪一个，若不明确可以询问用户。
...
async function runAgent(userText, port, session) {
  ...
  // 页面读不了时，执行模式只保留 navigate，以响应用户明确的打开网页请求；
  // 计划模式仍然不允许跳转。模型不会拿到读页面或其他操作工具。
  const defs = !pageAvailable
    ? mode === 'plan'
      ? []
      : TOOL_DEFS.filter((d) => d.name === 'navigate')
    : mode === 'plan'
