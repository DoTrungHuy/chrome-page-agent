...
const anthropic = {
  ...
  async stream({ config, system, systemVolatile, messages, tools, onEvent, signal }) {
    ...
    const blocks = [];
    let stopReason = null;
    // Anthropic 把输入 token 放在 message_start、输出 token 放在 message_delta
  ...
  async stream({ config, system, systemVolatile, messages, tools, onEvent, signal }) {
    ...
    const res = await fetch(`${trimSlash(config.baseUrl)}/chat/completions`, {
      ...
      body: JSON.stringify({
        ...
        // 不加这个的话，流式响应里根本不会带 usage。
        // 不是所有兼容端点都认，不认的会忽略掉，所以带上是安全的。
    ...
    // 带 stream_options.include_usage 时，usage 在最后一个分片里；
    // 有的端点会在每个分片都带一份，取最后见到的即可
    const usage = { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 };
    ...
    for await (const data of sseData(res)) {
      ...
      if (ev.usage) {
        ...
        // DeepSeek 等会额外报缓存命中
        usage.cacheRead = ev.usage.prompt_cache_hit_tokens
