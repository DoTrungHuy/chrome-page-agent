...
const BLOCKED = [
  ...
  'edge://', 'brave://', 'vivaldi://', 'opera://', 'opera-extension://',
  'moz-extension://', 'about:', 'devtools://', 'view-source:', 'file://',
];
...
export async function ensureContentScript(tabId) {
  ...
  try {
    const pong = await api.tabs.sendMessage(tabId, { type: 'ping' });
    if (pong?.ok) return;
  } catch {
    // 还没注入，往下走
  }
...
async function sendToPage(tabId, message) {
  let res;
  try {
    res = await api.tabs.sendMessage(tabId, message);
  } catch (e) {
...
async function navigate(tabId, url) {
  ...
  await api.tabs.update(tabId, { url: u.href });
